...
const ProductModal: React.FC<ProductModalProps> = ({ product, onClose, onAddToCart }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm animate-fade-in">
      <div className="w-full bg-surface rounded-t-[32px] max-h-[92vh] overflow-y-auto animate-slide-up shadow-2xl relative">
        <div className="relative h-64 w-full">
             <button onClick={onClose} className="absolute top-4 right-4 bg-white/30 backdrop-blur-md p-2 rounded-full text-white z-10 active:scale-90 transition-transform">
                <X size={20} />
             </button>
             <img src={`${IMG_BASE}${product.img}`} className="w-full h-full object-cover" alt={product.name} />
        ...
        <div className="px-6 pb-32 pt-6 space-y-8">
            ...
            <div className="pt-6 border-t border-gray-100">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setIsReviewFormOpen(!isReviewFormOpen)} className="text-accent text-sm font-semibold">
                ...
                {isReviewFormOpen && (
                    <div className="bg-gray-50 p-4 rounded-2xl mb-6 animate-scale-in">
                        ...
                        <div className="relative">
                            <input type="text" value={newReviewText} onChange={(e) => setNewReviewText(e.target.value)} placeholder="Ваше мнение о товаре..." className="w-full p-3 pr-10 rounded-xl border border-gray-200 focus:outline-none focus:border-accent text-sm" />
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 glass border-t border-gray-100/50">
            <button onClick={handleAdd} className="w-full bg-primary text-white h-14 rounded-2xl font-bold text-lg shadow-xl shadow-primary/20 flex justify-between px-6 items-center active:scale-[0.98] transition-all">
                <span>В корзину</span>
                <span>{getPrice()} ₽</span>
            </button>
        </div>
